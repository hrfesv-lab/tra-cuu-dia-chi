...
# ==========================================
# 3. BÓC TÁCH MỚI -> CŨ & TỰ SỬA LỖI AI
# ==========================================
...
def smart_ai_lookup(model, address_list, batch_size=5):
    ...
    for idx, batch in enumerate(batches):
        ...
        if prompt_data:
            prompt = f"""
            HỆ THỐNG KIỂM ĐỊNH VÀ CHUYỂN ĐỔI ĐỊA CHỈ VIỆT NAM (Mốc thời gian trước sáp nhập 2023-2025)
            
            DANH SÁCH CẦN XỬ LÝ (JSON):
            {json.dumps(prompt_data, ensure_ascii=False)}
            
            QUY TẮC XỬ LÝ:
            1. Nếu "type" == "merge_choice": Giữ nguyên "prefix", chọn 1 ĐỊA CHỈ CŨ trong "candidates" phù hợp nhất với tên đường/số nhà đó và ghép lại.
            2. Nếu "type" == "geocoding_fix": Địa chỉ này BỊ NGƯỜI DÙNG NHẬP SAI PHƯỜNG/XÃ (Nó không thuộc diện sáp nhập). BẠN HÃY DÙNG KIẾN THỨC BẢN ĐỒ VIỆT NAM để tra cứu xem "prefix" (Số nhà/Tên đường) kết hợp với Quận/Tỉnh của địa chỉ gốc thì TÊN PHƯỜNG/XÃ CHUẨN là gì. Tự động sửa lại tên Phường/Xã cho đúng thực tế.
            3. Tuyệt đối KHÔNG trả lại y nguyên địa chỉ bị sai.
            
            TRẢ VỀ JSON:
            Key là địa chỉ gốc. Value: {{"address": "KẾT QUẢ ĐỊA CHỈ HOÀN CHỈNH ĐÃ SỬA", "confidence": "Cao" hoặc "Nghi ngờ"}}
            """
    ...
    if sub_mode_ai == "Chuyển đổi hàng loạt":
        ...
        if st.button("⏪ Yêu cầu AI dịch ngược", type="primary", key="btn_ai"):
            if not selected_model: st.warning("⚠️ Vui lòng nhập API Key hợp lệ ở bảng cấu hình phía trên trước!")
                ...
                with st.spinner("Đang Dò Tìm Excel & AI Tự Sửa Lỗi Bản Đồ..."):
